...
- [ ] Record evaluation Set B (team webcam data) — **still the adoption criterion (§6.1); nothing ships on Set A alone.** Not possible right now (confirmed 2026-09-07). The recorder is written and waiting on a human: `uv run python scripts/setb.py record --person <name>`, then `... eval`
